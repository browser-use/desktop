...
type CodexLoginPtyArgs = string[] | string;
type CodexLoginPtyOptions = (pty.IPtyForkOptions | pty.IWindowsPtyForkOptions) & {
...
export function codexLoginPtySpawnSpec(
  ...
  return {
    ...
    args: resolved.spawnOptions.windowsVerbatimArguments ? resolved.args.join(' ') : resolved.args,
...
export function runCodexDeviceLogin(opts: CodexLoginOptions = {}): Promise<CodexLoginResult> {
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      const spawnOptions: CodexLoginPtyOptions = {
        name: 'xterm-256color',
        cols: 100,
        rows: 30,
        env: spec.env,
